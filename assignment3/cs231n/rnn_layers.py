...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        cur_x = x[:, t, :]
        prev_h, cache_tmp = rnn_step_forward(cur_x, prev_h, Wx, Wh, b)
        h[:, t, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx[:, t, :], dprev_h, dWx_tmp, dWh_tmp, db_tmp = rnn_step_backward(cur_dh, cache[t])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, b, prev_h, prev_c, input_gate, forget_gate, output_gate, 
             gate_gate, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, b, prev_h, prev_c, input_gate, forget_gate, output_gate, \
        gate_gate, next_c, next_h = cache
    ...
    # We add dnext_c and dtanh as during forward pass we split activation 
    ...
    da_i_partial = dtanh_dc * gate_gate
    ...
    da_g = (1-np.square(gate_gate)) * da_g_partial
...
def lstm_backward(dh, cache):
    ...
    x, Wx, Wh, b, prev_h, prev_c, input_gate, forget_gate, output_gate, \
        block_input, next_c, next_h = cache[0]
    ...
    for time_step in reversed(range(T)):
        ...
        dx[:,time_step,:], dprev_h, dprev_c, dWx_temp, dWh_temp, db_temp \
            = lstm_step_backward(cur_dh, dprev_c, cache[time_step])
